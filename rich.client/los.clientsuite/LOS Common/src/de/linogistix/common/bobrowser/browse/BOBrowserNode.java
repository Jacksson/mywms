...
//--------------------------------------------------------------------------
    ...
    public static class RootBONode extends BOBrowserNode {
        ...
        public RootBONode() throws DataObjectNotFoundException {
            super(DataObject.find(
                        Repository.getDefault().getDefaultFileSystem().getRoot().getFileObject(BOBROWSER_FOLDER)).getNodeDelegate());
    ...
    public static class BOFolderChildren extends FilterNode.Children {
        ...
        protected Node[] createNodes(Node key) {
            ...
            DataFolder folder = key.getLookup().lookup(DataFolder.class);
            DataObject dao = key.getLookup().lookup(DataObject.class);
...
//                    }
    ...
    //                    initMasterActions(bo);
        ...
        /** init every node to be sure that registration for BOLookup is done
         * for any subclass of BO and returns a list of those.
         */
        public static List<BO> getBOs() {
            try {
    ...
    //                            && exist.getClass().equals(bo.getClass())){
        ...
        public static void initMasterActions(BO bo) {
            ...
            try {
                ...
                FileObject fob = Repository.getDefault().getDefaultFileSystem().getRoot().getFileObject(masterActionsName);
                if (fob != null) {
                ...
                fob = Repository.getDefault().getDefaultFileSystem().getRoot().getFileObject(masterActionsName);
                if (fob != null) {
                    masterActions = DataObject.find(fob);
                    masterActionFolder = masterActions.getLookup().lookup(DataFolder.class);
                    for (DataObject o : masterActionFolder.getChildren()) {
        ...
        public static void initActions(BO bo) {
            ...
            try {
                ...
                fob = Repository.getDefault().getDefaultFileSystem().getRoot().getFileObject(actionsName);
                if (fob != null) {
                    actions = DataObject.find(fob);
                    actionFolder = actions.getLookup().lookup(DataFolder.class);
                    for (DataObject o : actionFolder.getChildren()) {
    ...
    static class DAORecurser {
        ...
        DAORecurser() {
            try {
                ...
                DataObject dao = DataObject.find(Repository.getDefault().getDefaultFileSystem().getRoot().getFileObject(BOBROWSER_FOLDER));
                recurse(dao);
