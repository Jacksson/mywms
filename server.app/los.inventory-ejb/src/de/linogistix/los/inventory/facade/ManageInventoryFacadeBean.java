...
//    
//    @EJB
//    LOSInventoryComponent invComponent;
    ...
    {    
        ...
        inventoryComponent.createStockUnitOnStorageLocation(clientRef, slName, articleRef, lotRef, amount, unitLoadRef,genService.generateManageInventoryNumber(u), null );
    }
    ...
    public void deleteStockUnitsFromStorageLocations(List<BODTO<LOSStorageLocation>> locations) throws FacadeException{
    	...
    	for (BODTO<LOSStorageLocation> loc : locations){
    		...
    		if (sl != null){
    			inventoryComponent.deleteStockUnitsFromStorageLocation(sl, genService.generateManageInventoryNumber(u));
    		}
    	}
    }
    ...
    public void sendStockUnitsToNirwanaFromSl(List<BODTO<LOSStorageLocation>> locations) throws FacadeException{
    	...
    	for (BODTO<LOSStorageLocation> loc : locations){
    		LOSStorageLocation sl = manager.find(LOSStorageLocation.class, loc.getId());
    		if (sl != null){
    			inventoryComponent.sendStockUnitsToNirwana(sl, genService.generateManageInventoryNumber(u));
    		}
    	}
    }
    ...
    public void sendStockUnitsToNirwana(List<BODTO<StockUnit>> sus) throws FacadeException{
    	...
    	for (BODTO<StockUnit> loc : sus){
    		...
    		if (su != null){
                    inventoryComponent.changeAmount(su, new BigDecimal(0), true, genService.generateManageInventoryNumber( u), null, null, true);
                    inventoryComponent.sendStockUnitsToNirwana(su, genService.generateManageInventoryNumber(u));
    		}
    	}
    }
    ...
    public void sendStockUnitsToNirwanaFromUl(List<BODTO<LOSUnitLoad>> unitLoads) throws FacadeException{
    	...
    	for (BODTO<LOSUnitLoad> loc : unitLoads){
    		...
    		if (ul != null){
    			inventoryComponent.sendStockUnitsToNirwana(ul, genService.generateManageInventoryNumber(u));
    		}
    	}
    }
	...
	public void transferStockUnit(BODTO<StockUnit> suTO,
		...
		User u = contextService.getCallersUser();
		String s = genService.generateManageInventoryNumber(u);
		if (removeSuLock){
			su.setLock(0);
		}
	...
	public void transferStock(BODTO<LOSUnitLoad> from, BODTO<LOSUnitLoad> to
		...
		inventoryComponent.transferStock(fromUl, toUl, s);		
	}
	...
	public void transferUnitLoad(BODTO<LOSStorageLocation> target,
		...
		if( fix != null ) {
			...
			if (targetLocation.getUnitLoads() != null && targetLocation.getUnitLoads().size() > 0) {
				...
				inventoryComponent.transferStock(unitLoad, onDestination, "");
				storage.sendToNirwana( contextService.getCallersUser().getName(), unitLoad);
				...
				storage.transferUnitLoad( u.getName(), targetLocation, unitLoad, -1, false, null, null);
		...
		storage.transferUnitLoad(u.getName(), targetLocation, unitLoad, -1, ignoreSlLock, info, "");
