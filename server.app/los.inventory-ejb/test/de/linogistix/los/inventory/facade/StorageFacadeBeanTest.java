...
//		q.addWhereToken(tc);
			...
			if (ul.getStockUnitList().size() > 0) {
				for (StockUnit su : ul.getStockUnitList()){
					...
					if (su.getClient().equals(
						...
						if (ul.getStorageLocation().getArea().getAreaType().equals(
		...
		for (String ulLabel : labels) {
			// Test normal process
			label = ulLabel;
			LOSStorageRequest r = bean.getStorageRequest(label);
			assertNotNull(r);
			...
			if (su.getLot().getName().equals(InventoryTestTopologyRemote.LOT_N1_A1_NAME)) {
				...
				assertTrue(r.getDestination().getReservedCapacity() == 1);
				...
				assertTrue(sl.getReservedCapacity() == 0);
