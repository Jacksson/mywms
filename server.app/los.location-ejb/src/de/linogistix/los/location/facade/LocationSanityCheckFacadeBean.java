...
public class LocationSanityCheckFacadeBean implements LocationSanityCheckFacade {
	...
	public void adjustReservation(){
		
		QueryDetail d= new QueryDetail(0, Integer.MAX_VALUE);
		TemplateQueryWhereToken t= new TemplateQueryWhereToken(TemplateQueryWhereToken.OPERATOR_SMALLER, "reservedCapacity", 0);
		TemplateQuery q = new TemplateQuery();
		q.addWhereToken(t);
		q.setBoClass(LOSStorageLocation.class);
		List<BODTO<LOSStorageLocation>> sls;
		try {
			sls = slQueryRemote.queryByTemplateHandles(d, q );
		} catch (Throwable e) {
		...
		for (BODTO<LOSStorageLocation> to : sls){
			...
			if (sl.getReservedCapacity() < 0){
				log.error("Negative reservation. Setting to 0: " + sl.toDescriptiveString());
