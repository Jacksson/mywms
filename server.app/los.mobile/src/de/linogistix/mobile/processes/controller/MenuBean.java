...
public class MenuBean extends BasicBackingBean {
    ...
    private void init() {
    	...
    	for( MobileFunction fct : fctList ) {
			...
			try {
				...
				log.error("Cannot instanciate dialog. Name="+fct.getClassName()+", Exception: " + e.getMessage());
				continue;
			}
			if( dialog == null ) {
    ...
    private void resetProcesses() {
        JSFHelper.getInstance().resetBean(de.linogistix.mobile.processes.picking.chooseOrder.gui.bean.ComboBoxBean.class);
