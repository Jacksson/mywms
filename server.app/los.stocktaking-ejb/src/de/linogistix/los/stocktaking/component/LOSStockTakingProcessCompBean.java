...
public class LOSStockTakingProcessCompBean implements LOSStockTakingProcessComp {
	...
	public LOSStockTaking startNewStockTaking(LOSStockTakingType type) {
		...
		LOSStockTaking st = new LOSStockTaking();

		long sn = seqService.getNextSequenceNumber(LOSStockTaking.class);
		String stNo = "IV " + String.format("%08d", sn);
		...
		if (type == LOSStockTakingType.END_OF_PERIOD_INVENTORY) {
			...
			List<LOSStorageLocation> slList = queryLocService
					.getListByAreaType(LOSAreaType.STORE);
	...
	public LOSStocktakingOrder createStockTakingOrder(LOSStockTaking st,
		...
		if( orders == null || orders.size() == 0 ) {
			LOSStocktakingOrder so = new LOSStocktakingOrder();
	...
	public void processLocationEmpty(LOSStorageLocation sl)
		...
		if (ulList.size() == 0) {
			...
			for (LOSStocktakingOrder so : soList) {
				...
				LOSStocktakingRecord rec = new LOSStocktakingRecord();
...
//			}
	...
	public void recountOrder( long orderId ) throws LOSStockTakingException {
        ...
        LOSStocktakingOrder stOrderNew = new LOSStocktakingOrder();
	...
	public int generateOrders(
		...
		sb.append("SELECT distinct sl FROM " +LOSStorageLocation.class.getSimpleName() + " sl WHERE lock=0 ");
